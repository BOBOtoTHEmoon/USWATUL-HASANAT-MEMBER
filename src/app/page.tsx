...
/* ─── MEMBER ROW ─── */
function MemberRow({ member, onSelect, onDelete }: { member: Member; onSelect: (m: Member) => void; onDelete: (m: Member) => void }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 px-4 py-3.5 flex items-center gap-3 hover:border-gray-200 transition-colors">
      ...
      <button onClick={() => onSelect(member)} className="text-xs text-green-600 font-medium hover:text-green-800 px-3 py-1.5 rounded-lg hover:bg-green-50 transition-colors">QR</button>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-sm w-full p-6 text-center" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-white p-4 inline-block rounded-xl border border-gray-100">
          <QRCode value={url} size={200} fgColor="#15803d" />
        </div>
        <p className="text-xs text-gray-400 mt-3 font-mono break-all">{url}</p>
        <p className="text-xs text-gray-500 mt-4">Right-click the QR code to save as image</p>
      </div>
    </div>
  );
}
        ...
        {members.map((m) => (
          <div key={m.id} className="bg-white rounded-xl border border-gray-100 p-4 text-center">
            <div className="inline-block">
              <QRCode value={`${base}/member/${m.reg_number}`} size={110} fgColor="#15803d" />
            </div>
      ...
      {tab === "members" && (
        <>
          ...
          {loading ? (
            ...
            <div className="space-y-2">{filtered.map((m) => <MemberRow key={m.id} member={m} onSelect={setQrMember} onDelete={deleteMember} />)}</div>
          )}
        </>
      )}
